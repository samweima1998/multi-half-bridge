...
void executePattern(std::vector<Tle94112Rpi>& controllers, Tle94112::HalfBridge* hbPins, Tle94112::HBState* states) {
    while (true) {
        if (running.load()) {
            ...
            std::this_thread::sleep_for(std::chrono::milliseconds(200)); // Cycle delay
        } else {
...
int main() {
    ...
    for (int i = 0; i < 8; i++) controllers[i].begin();
    for (int i = 0; i < 8; i++) controllers[i].cs->init();
    for (int i = 0; i < 8; i++) {
    ...
    std::cout << "SHUTDOWN\n" << std::flush;
    for (int i = 0; i < 8; i++) controllers[i].end();
    bcm2835_close();
    return 0;
}
